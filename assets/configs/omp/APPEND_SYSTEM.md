...
- `mcp__codegraph_explore` — codegraph's SOLE MCP tool in this build: ONE call = relevant source + call paths + blast-radius (folds callers/callees/impact). Run `codegraph index .` once if `.codegraph/` is missing. Need one precise slice instead? Shell out via `bash`: `codegraph callers|callees|impact|node|query|status <sym>` — these are CLI verbs, NOT MCP tools; never call `mcp__codegraph__callers` (does not exist).
...
**Full catalogs are visible:** ALL MCP tools of `codebase-memory-mcp` · `serena` · `headroom` are in your tool list — cbm cypher (`query_graph`)/`detect_changes`, serena edit/rename included. codegraph exposes exactly ONE MCP tool (`mcp__codegraph_explore`); its callers/callees/impact/node/query/status are `codegraph` CLI verbs (run via `bash`), not MCP tools. Only OTHER/newly-added MCP servers' tools hide behind ONE `search_tool_bm25` call — search before concluding a tool doesn't exist.
