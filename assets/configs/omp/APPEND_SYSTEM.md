...
omp (`tools.xdev: true`) mounts MCP tools as `xd://` devices — they are NOT top-level tools. Call one by writing its JSON args to the device: `write` path `xd://mcp__codegraph_explore`, content `{"query":"…"}`. `8sync harness` sets `tools.xdevDocs: builtins` + `tools.xdevInlineDevices`, so the devices below ship full docs/schemas in the prompt; any other MCP tool: `read xd://<tool>` for its schema first.
- `xd://mcp__codegraph_explore` — ONE call = relevant source + call paths + blast-radius. Run `codegraph index .` once if `.codegraph/` is missing. One precise slice instead? `bash`: `codegraph callers|callees|impact|node|query|status <sym>` (CLI verbs, not devices).
- `xd://mcp__codebase_memory_mcp_search_graph` · `…_trace_path` · `…_get_architecture` · `…_get_code_snippet` · `…_search_code` — where is X · who calls X · impact · architecture.
- `xd://mcp__serena_find_symbol` · `…_find_referencing_symbols` · `…_get_symbols_overview` — LSP-precise symbol lookup; find references BEFORE editing an exported symbol (built-in `lsp` works too).
Routing: symbol / structure / call-path / impact questions → codegraph · cbm · serena FIRST. `grep`/`glob` only for plain-text lookup where structure is irrelevant. `read` a raw file only when about to edit it. The code-intel hook blocks the first `grep`/`read` on code per prompt until one of these ran — retry the same call if text search really is the right tool; that unlocks the rest of the prompt.
...
- **`recall` / `reflect` BEFORE** answering anything about past sessions, decisions, or user prefs; **`retain`** durable facts (decisions, conventions, prefs) AFTER. omp Mnemopi long-term memory — the recall hook also auto-injects the live STATE (Current/Next) every prompt; omp itself lists every skill.
...
- Context auto-compacts at the configured threshold (`8sync harness compaction <pct>`) — write a handoff into STATE before it fires. This block is never compressed, so it stays terse by design.
