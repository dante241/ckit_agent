...
> Bước 1/4/5 (validate slug, tạo file từ template, set ACTIVE + config) cũng làm được **deterministic** bằng verb `ckit feature new <slug>` (nhanh, không cần model). `/feature new` trong session dùng khi cần thêm phán đoán (spec ở đâu, cắt phase). Cả hai đều thay placeholder `SLUG`/`FEATURE_NAME`/`DATE`.
