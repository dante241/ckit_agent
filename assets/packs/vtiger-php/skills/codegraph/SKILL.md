...
You are running inside an ckit project that already has `codegraph` installed and a `.codegraph/` index. **Use it.** Dumping whole files into context is wasteful when the index can answer in one call.
...
If `.codegraph/` does NOT exist in cwd → `codegraph init .` then `codegraph index .`. `ckit skill sync` already does this for you when run inside a project; you only need to re-init if the directory has been wiped.
...
- `codegraph init -i` — init + initial index in one shot (ckit already does the init half).
- `codegraph serve` — MCP server mode for agents that speak MCP (rarely needed from inside ckit).
- **Graph as ONE image** (vision models / layout questions): `ckit shot 'http://127.0.0.1:8731/codegraph?shot=1' -o /tmp/cg.png` — canvas-only React-Flow package graph (`ckit harness web` must be running). Details stay text: `/api/codegraph/overview|search|trace`. Non-vision models read positions from the shot via `ckit locate` (automatic).
...
Use codegraph to **locate**, then `Read` (or ckit's `read`) to fetch the narrow slice you actually need. The pattern is:
