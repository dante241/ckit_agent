...
## validated: codegraph exposes ONE MCP tool; other verbs are CLI (2026-08-06)
- Grounded `tools/list` (JSON-RPC to `codegraph serve --mcp`) on the installed codegraph shows it registers **exactly one** MCP tool: `codegraph_explore`. Not `codegraph_node`, and NOT callers/callees/impact/status/search/files as MCP tools (those appear in upstream README's Claude-Code allowlist but are consolidated away in this build). They exist only as `codegraph` **CLI verbs** (run via bash).
- omp's callable id for it is single-underscore `mcp__codegraph_explore` (matches `config.yml` xdevInlineDevices + the MCP route table), NOT the double-underscore `mcp__codegraph__codegraph_explore` MCP convention. APPEND_SYSTEM RULE #0 had the wrong name + falsely listed callers/impact/status as MCP catalog → agent invented non-existent `mcp__codegraph__callers` and bailed.
- Fix (assets/configs/omp/APPEND_SYSTEM.md lines 7 & 12): name the sole MCP tool correctly; route precise single-slice lookups (`codegraph callers|callees|impact|node|query|status <sym>`) to the CLI via bash. Rebuild embeds the asset; `ckit harness global` redeploys to `~/.omp/agent/APPEND_SYSTEM.md` (byte-stable). To probe any MCP server's real tool surface: pipe initialize + notifications/initialized + tools/list into `<server> --mcp` and parse the id==tools/list response.

## validated: harness seeds tools.xdevInlineDevices on omp ≥17 (2026-08-06)
- Before: `ensure_mcp_tools_visible` no-op'd for omp ≥17, so `tools.xdev`/`xdevInlineDevices` were a manual config.yml hand-edit → fresh machines never got inline schemas → model didn't auto-reach codegraph/cbm/serena. Now `ensure_xdev_inline_devices` (deploy.rs) seeds the 9 code-intel devices, key-presence idempotent, inserts UNDER an existing `tools:` block (preserves user `approvalMode`). Smoke-tested 3 paths: insert-under-tools, idempotent re-run (count stays 1), append-fresh-tools.
- `approvalMode: yolo` is deliberately NOT seeded — auto-approving every tool machine-wide is a security/autonomy choice; documented as a manual opt-in in CHANGELOG. cbm entries use the glob `mcp__codebase_memory*_...` so they match whether the server registers as `codebase-memory-mcp` or `codebase-memory`.
